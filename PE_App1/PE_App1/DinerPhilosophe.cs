...
namespace PE_App1
{
    ...
    public class EcranDinerPhilosophes : System.Windows.Forms.Form
	{
		...
		public EcranDinerPhilosophes()
		{
			...
			for (int i = 0; i < this.nbPhilosophes; i++)
			{
				...
				gestion[i] = new Thread(new ThreadStart(this.philosophes[i].ActionPhilosophe)); //pointeur vers "début" programme
		...
		protected override void Dispose(bool disposing) //Appelée automatiquement par le framework à la fermeture
		{
			if( disposing )
			{
				if(components != null)
		...
		private void EcranDinerPhilosophes_Paint(object sender, System.Windows.Forms.PaintEventArgs e) //Dessin de base
		{
			Graphics gr = CreateGraphics();
			int xc = this.ClientSize.Width/2;
			int yc = this.ClientSize.Height/2;
			gr.FillEllipse(new SolidBrush(Color.Goldenrod),xc-80,yc-80,160,160);
			for(int i=0;i<this.nbPhilosophes;i++)
			{
				...
				gr.FillEllipse(new SolidBrush(Color.Goldenrod)
		...
		public void Avancer(int numero) //Juste dessin
		{
		...
		public void Reculer(int numero) //Juste dessin
		{
		...
		public void PeutManger(int numero)
		{
            ...
            //Attend les 2 baguettes
			this.baguettes[numero].WaitOne();
			this.baguettes[(numero+1)%nbPhilosophes].WaitOne();
			foc.WriteLine("Entrée en Lock de "+numero.ToString()); //Fichier de logs
		...
		public void Deposerbaguettes(int numero)
		{
            ...
            //Relache les 2 Mutex quand le moine à fini de manger
	...
	class Philosophes
	{
		...
		private void Manger()
		{
			...
			Application.DoEvents(); //Sort de la boucle si l'event est activé
